...
description: Validate fetched data at runtime and buildtime
...
> fetching in TypeScript applications. This article is part one, describing
> common failures to address type safety when fetching data, then I'll continue
> with testing strategies that work well with TypeScript
...
using `as` &mdash; when `as` as used, this is a "trust me, bro" moment. Don't
worry your pretty little head about the type of the response, "Trust me, bro"
&mdash; the type of the response is _definitely_ what I say it is no matter
what.
...
`zod` (and other similar tools, like [valibot](https://valibot.dev)) provide an
...
data fetching in TypeScript applications. Next, I'll cover pairing our runtime
schema validation with data mocking to author automated integration tests
without flakiness typical of other approaches.
