...
are a language feature that should be avoided at all costs. Though seemingly
offering parity with other typed languages, this feature worsens developer
experience and can be replaced by string unions in most cases.

Often used for things like prop types, an enum looks something like this in a
React context:
...
By the time TypeScript compiles, JavaScript no longer has a reference to the
enum in any 1 to 1 sense. As a result, an "enum" becomes an object at runtime.
This isn't necessarily a _problem_ but it can be confusing, particularly to
developers who may not be aware that all constructs in TypeScript to not map 1
to 1 with a JavaScript equivalent.

### 2. Enums add to bundle size

Because an `enum` is used as a value, it must be available at runtime, adding to
overall bundle size once compiled.
...
only use case is within the `template`.
...
Not to mention, now the `App` class exposes a public `Variant` that can be
...
and derive the time from said array.
